---
title: 2FA、无密码和身份验证的未来
date: 2026-01-01 15:33
tags: [前端]
excerpt: 浅谈互联网身份验证技术的发展。
id: future-login
---

## 从 SFA（单因素验证）到 2FA（双因素验证）
...
![Chrome 生成密码](./future-login/chrome-password-generate.png)
...
上面提到的短信验证码和 Steam 令牌都属于 **OTP**[^3]、即 **一次性密码**，是 2FA 中的一种因素，通过一个具有时效性的密码来验证用户身份，这是一种被广泛使用的验证方式，银行 U 盾（硬件令牌）生成的验证码也属于 OTP。
...
2. **TOTP** 基于 HOTP，在 TOTP 中，变化的 C 值是由当前时间计算得到的。这是 [一种 HOTP / TOTP 算法的 JavaScript 实现](./future-login/totp.html) 可供体验：
...
![Passkey](./future-login/passkey.png)
